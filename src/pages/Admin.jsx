                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {u.role === "admin" && (
                          <Badge className="text-[10px] uppercase tracking-wide">Admin</Badge>
                        )}
                        {me?.id === u.id && (
                          <Badge
                            variant="secondary"
                      ...
                      <div className="text-[11px] text-muted-foreground/80 mt-0.5">
                        Joined {formatDate(u.createdAt)} · {u._count?.meals ?? 0} meals ·{" "}
                        {u._count?.workouts ?? 0} workouts · {u._count?.waterEntries ?? 0} water ·{" "}
                        {u._count?.healthData ?? 0} health
                      </div>
                    </div>
                  </button>
                </li>
  ...
  function handleResetPassword(e) {
    ...
    resetMutation.mutate(
      { id: target.id, newPassword },
      {
        onSuccess: () => {
          setNewPassword("");
          toast.success("Password updated. Share it out of band.");
        },
