...
const CATEGORY_COLORS = {
  "Upper Body Push": {
    ...
    tab: "border-indigo-600 text-indigo-600 dark:border-indigo-400 dark:text-indigo-400",
  },
  "Upper Body Pull": {
    dot: "bg-emerald-500",
    text: "text-emerald-600 dark:text-emerald-400",
    badge: "bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300",
    tab: "border-emerald-600 text-emerald-600 dark:border-emerald-400 dark:text-emerald-400",
  },
  "Lower Body": {
    dot: "bg-amber-500",
    text: "text-amber-600 dark:text-amber-400",
    badge: "bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300",
    tab: "border-amber-600 text-amber-600 dark:border-amber-400 dark:text-amber-400",
  },
  Core: {
    dot: "bg-violet-500",
    text: "text-violet-600 dark:text-violet-400",
    badge: "bg-violet-50 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300",
    tab: "border-violet-600 text-violet-600 dark:border-violet-400 dark:text-violet-400",
  },
  Cardio: {
    dot: "bg-rose-500",
    text: "text-rose-600 dark:text-rose-400",
    badge: "bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300",
    tab: "border-rose-600 text-rose-600 dark:border-rose-400 dark:text-rose-400",
  },
};
...
export default function Workouts() {
  ...
  const saveError =
    saveClientError || (logWorkoutMutation.error && logWorkoutMutation.error.message) || "";

  const quickSaving = quickLogMutation.isPending;
  const quickError =
    quickClientError || (quickLogMutation.error && quickLogMutation.error.message) || "";
  ...
  const presetSaving =
    createWorkoutPresetMutation.isPending || updateWorkoutPresetMutation.isPending;
  const presetError =
    presetClientError ||
  ...
  const saveAsPresetErr =
    saveAsPresetClientErr ||
    (saveAsPresetMutation.error && saveAsPresetMutation.error.message) ||
    "";
  ...
  function handleSaveAsPreset() {
    if (!saveAsPresetName.trim()) {
      setSaveAsPresetClientErr("Enter a preset name.");
      return;
    }
  ...
  function handleQuickLog() {
    ...
    const splitDay =
      cats.length === 1
        ? cats[0].toLowerCase().replace(/ /g, "_")
        : cats.map((c) => c.toLowerCase().replace(/ /g, "_")).join("+");
    quickLogMutation.mutate(
      { date: selectedDate, splitDay, exercises: [] },
      {
        onSuccess: () => {
  ...
  function handleCreateExercise() {
    if (!exForm.name.trim()) {
      setExClientError("Name is required.");
      return;
    }
    const muscles = exForm.muscles
      .split(",")
      .map((m) => m.trim())
      .filter(Boolean);
    if (!muscles.length) {
      setExClientError("Enter at least one muscle.");
      return;
    }
    setExClientError("");
  ...
  function togglePresetExercise(exercise) {
    if (presetPlanIds.has(exercise.id))
      setPresetPlan((prev) => prev.filter((e) => e.id !== exercise.id));
    else setPresetPlan((prev) => [...prev, exercise]);
  }

  function handleSavePreset() {
    if (!presetName.trim()) {
      setPresetClientError("Enter a preset name.");
      return;
    }
    setPresetClientError("");
    const payload = {
      name: presetName.trim(),
      exercises: presetPlan.map((e) => ({
        id: e.id,
        name: e.name,
        category: e.category,
        muscles: e.muscles,
  ...
  const visiblePresetExercises =
    presetCategory === "All"
      ? allExercises
      : allExercises.filter((e) => e.category === presetCategory);
    ...
    <div className="space-y-6">
      ...
      <Card className="py-0 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          ...
          <div className="flex gap-2">
            <Button type="button" variant="outline" size="sm" onClick={openPresetsPanel}>
              Presets
            </Button>
            <Button
              type="button"
        ...
        <div className="px-5 py-4">
          {plan.length === 0 ? (
            <p className="text-sm text-muted-foreground py-2">
              Your plan is empty — browse exercises below to build your workout.
            </p>
          ) : (
          ...
          <div className="px-5 pb-5 space-y-3">
            ...
            {saveAsPreset ? (
              ...
              <Button
                type="button"
                variant="outline"
                onClick={() => setSaveAsPreset(true)}
                className="w-full text-primary border-primary/30 hover:border-primary hover:text-primary"
              >
                Save Plan as Preset
              </Button>
            )}
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Tap the categories you trained today without selecting individual exercises.
            </p>
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Button type="button" variant="outline" size="sm" onClick={openNewExercisePanel}>
            + Custom Exercise
          </Button>
        </div>
              ...
              >
                ...
                {exercise.isCustom && (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDeleteCustomExercise(exercise.dbId || exercise.id);
                    }}
                ...
                <p
                  className={cn("text-xs font-medium mb-1", colors.text || "text-muted-foreground")}
                >
                  {exercise.category}
                </p>
                {exercise.isCustom && (
                  <span className="inline-block text-xs font-medium bg-muted text-muted-foreground rounded-sm px-1.5 py-0.5 mb-1">
                    Custom
                  </span>
                )}
                ...
                return (
                  <li key={session.id || idx} className="px-5 py-4">
                    ...
                    {exerciseNames ? (
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Category log — no exercises recorded
                      </p>
                    )}
                  </li>
                );
      ...
      <Dialog open={showGenerator} onOpenChange={setShowGenerator}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Select the categories you want to train, then adjust the exercise count for each.
            </DialogDescription>
          </DialogHeader>
                ...
                return (
                  ...
                  >
                    <span
                      className={cn(
                        "size-2 rounded-full shrink-0",
                        isOn ? colors.dot : "bg-muted-foreground/40"
                      )}
                    />
                    {cat}
                  </button>
                  ...
                  return (
                    <div key={cat} className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        />
                        <span
                          className={cn("text-sm font-medium", colors.text || "text-foreground")}
                        >
                          {cat}
                        </span>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setGenCounts((prev) => ({
                              ...prev,
                              [cat]: Math.min(pool, (prev[cat] || 1) + 1),
                            }))
                          }
                          aria-label={`More ${cat}`}
                        >
                          ...
                          <div>
                            ...
                            {exList.length > 0 && (
                              <p className="text-xs text-muted-foreground/80 mt-1">
                                {truncate(exList.map((e) => e.name).join(", "), 80)}
                              </p>
                            )}
                          </div>
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              variant="outline"
                              onClick={() => openNewPresetPanel(preset)}
                            >
                              Edit
                            </Button>
                            <Button
                    ...
                    {presetPlan.map((e) => {
                      ...
                      return (
                        <div key={e.id} className="flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => togglePresetExercise(e)}
